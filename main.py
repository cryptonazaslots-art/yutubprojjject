...
async def fabricar_video():
    # 1. Guion - URL universal v1
    api_key = os.getenv('GEMINI_API')
    url = f"https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash-latest:generateContent?key={api_key}"
    
    payload = {
        "contents": [{
            "parts": [{"text": "Escribe un dato curioso del espacio de 35 palabras. Solo el texto."}]
        }]
    }
    ...
    if 'candidates' not in res:
        ...
        # Si falla el flash, intentamos con el pro como último recurso
        url_alt = f"https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key={api_key}"
        res = requests.post(url_alt, json=payload).json()
        if 'candidates' not in res:
            print("Fallo total de modelos.")
            return
    ...
    yt.videos().insert(
        part="snippet,status",
        body={
            "snippet": {"title": "Dato Espacial #shorts", "description": "#space", "categoryId": "22"},
